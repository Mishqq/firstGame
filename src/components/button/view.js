...
import {spriteStore, gameSounds, touchEvents} from './../../constants/presets';
...
export default class ButtonView {
	constructor(config) {
		...
		for(let key in config){
			...
			touchEvents.tap.forEach(event=>_b.on(event, this.onClick, this));

			touchEvents.start.forEach(event=>_b.on(event, this.btnSelect, this));
	...
	lockCancel(lockStatus){
		let btn = this._buttons.btnCancel;
